...
public class SolicitacaoService {
    ...
    public SolicitacaoResponse criarApo(SolicitacaoRequest req, UsuarioPrincipal principal) {
        UsuarioV2 aluno = usuarioV2Repo.findById(req.getAlunoId())
                .orElseThrow(() -> new ResourceNotFoundException("Aluno não encontrado"));

        Solicitacao sol = new Solicitacao();
        sol.setTitulo(req.getTitulo());
        sol.setDescricao(req.getDescricao());
        sol.setAlunoId(aluno.getId());
        sol.setNomeAluno(aluno.getNome());
        ...
        salva = solicitacaoRepo.save(salva);
    ...
    public Page<SolicitacaoResponse> listarPorStatusPaginado(String status, Pageable pageable) {
        return solicitacaoRepo.findByStatus(parseStatus(status), pageable).map(this::toResponse);
    }

    public Page<SolicitacaoResponse> listarPorOrientador(String orientadorId, Pageable pageable) {
        List<Solicitacao> todos = solicitacaoRepo.findByOrientadorId(orientadorId);
    ...
    public List<SolicitacaoResponse> listarPorAluno(String alunoId, UsuarioPrincipal principal) {
        if (!principal.id().equals(alunoId) && !principal.admin())
            throw new AccessDeniedException("Acesso negado");
        return solicitacaoRepo.findByAlunoId(alunoId).stream()
                .map(this::toResponse).collect(Collectors.toList());
    }

    public List<SolicitacaoResponse> listarPorAlunoV2(String alunoId, UsuarioPrincipal principal) {
        return solicitacaoRepo.findByAlunoId(alunoId).stream()
                .map(this::toResponse).collect(Collectors.toList());
    }

    public SolicitacaoResponse buscarPorId(String id, UsuarioPrincipal principal) {
    ...
    public SolicitacaoResponse buscarPorId(Long id, UsuarioPrincipal principal) {
        return buscarPorId(String.valueOf(id), principal);
    }

    // ── Edição ─────────────────────────────────────────────────────────────────

    public SolicitacaoResponse atualizarFormulario(String id, FormularioRequest req,
                                                    UsuarioPrincipal principal) {
        Solicitacao sol = findById(id);
        verificarDono(sol, principal);
        boolean podeEditar = sol.getStatus() == StatusSolicitacao.RASCUNHO
                || sol.getStatus() == StatusSolicitacao.EM_PREENCHIMENTO
                || sol.getStatus() == StatusSolicitacao.DEVOLVIDA_ALUNO;
        if (!podeEditar)
            throw new BusinessException("APO só pode ser editada em rascunho ou quando devolvida");
    ...
    public SolicitacaoResponse atualizarFormulario(Long id, FormularioRequest req,
                                                    UsuarioPrincipal principal) {
        return atualizarFormulario(String.valueOf(id), req, principal);
    }

    public SolicitacaoResponse enviarParaAnalise(String id, UsuarioPrincipal principal) {
        Solicitacao sol = findById(id);
        verificarDono(sol, principal);
    ...
    public SolicitacaoResponse enviarParaAnalise(Long id, UsuarioPrincipal principal) {
        return enviarParaAnalise(String.valueOf(id), principal);
    }

    public SolicitacaoResponse atualizarStatus(String id, String novoStatus, UsuarioPrincipal principal) {
        Solicitacao sol = findById(id);
    ...
    public SolicitacaoResponse definirResultadoFinal(Long id, String status, Double pontuacao,
                                                      UsuarioPrincipal principal) {
        return definirResultadoFinal(String.valueOf(id), status, pontuacao, principal);
    }

    public SolicitacaoResponse definirResultadoFinal(String id, String status, Double pontuacao,
                                                       UsuarioPrincipal principal) {
        Solicitacao sol = findById(id);
    ...
    public SolicitacaoResponse darAvalFinal(Long id, boolean aprovado, String justificativa,
                                             UsuarioPrincipal principal) {
        return darAvalFinal(String.valueOf(id), aprovado, justificativa, principal);
    }

    public SolicitacaoResponse darAvalFinal(String id, boolean aprovado, String justificativa,
                                             UsuarioPrincipal principal) {
        Solicitacao sol = findById(id);
        if (!sol.getStatus().aguardandoComissao())
            throw new BusinessException("Aval final só é possível quando aguardando comissão");
    ...
    public SolicitacaoResponse transicionarStatus(String id, StatusSolicitacao destino,
                                                   UsuarioPrincipal principal,
    ...
    public SolicitacaoResponse comissaoAprovar(String id, String novoTipoApoId,
    ...
    public SolicitacaoResponse coordenacaoAprovar(String id, String novoTipoApoId,
                                                   String justificativa, UsuarioPrincipal principal) {
        if (novoTipoApoId != null) {
            Solicitacao sol = findById(id);
            tipoApoRepo.findById(novoTipoApoId).ifPresent(t -> {
    ...
    public SolicitacaoResponse lancarNoSistema(String id, UsuarioPrincipal principal) {
        Solicitacao sol = findById(id);
        if (sol.getStatus() != StatusSolicitacao.ARQUIVADA)
            throw new BusinessException("APO precisa estar arquivada antes de ser lançada");
        SolicitacaoResponse resp = transicionarStatus(id, StatusSolicitacao.LANCADA_SISTEMA, principal, "LANCADA_SISTEMA", null);
        atualizarCreditosAluno(sol.getAlunoId());
        return resp;
    }

    // ── Créditos ───────────────────────────────────────────────────────────────

    public int calcularCreditosAprovados(String alunoId) {
        return solicitacaoRepo.findByAlunoId(alunoId).stream()
                .filter(s -> List.of("LANCADA_SISTEMA","FINALIZADA","ARQUIVADA","APROVADA_FINAL","APROVADA")
                        .contains(s.getStatus().name()))
                .mapToInt(s -> s.getCreditosAprovados() != null ? s.getCreditosAprovados() : 0)
                .sum();
    }

    public List<Map<String, Object>> resumoCreditosAlunos(String tipoAluno, String orientadorId) {
        ...
        usuarioV2Repo.findAll().stream()
                ...
                .forEach(u -> {
                    int necessarios = calcularCreditosNecessarios(u.getTipoAluno());
                    int acumulados  = u.getCreditosAprovados() != null ? u.getCreditosAprovados() : 0;
                    ...
                    item.put("creditosAprovados",   acumulados);
                    item.put("creditosNecessarios", necessarios);
                    item.put("completo",            acumulados >= necessarios && necessarios > 0);
                    resultado.add(item);
                });
        return resultado;
    }
    ...
    public List<SolicitacaoResponse> listarPorStatus(String status) {
        return solicitacaoRepo.findByStatus(parseStatus(status)).stream()
                .map(this::toResponse).collect(Collectors.toList());
    }
    ...
    private void atualizarResponsavel(Solicitacao sol, StatusSolicitacao status) {
        String resp = switch (status) {
            case RASCUNHO, DEVOLVIDA_ALUNO, EM_PREENCHIMENTO -> "ALUNO";
    ...
    private void registrarHistorico(Solicitacao sol, UsuarioPrincipal principal,
        ...
        if (sol.getHistorico() == null) sol.setHistorico(new ArrayList<>());
        sol.getHistorico().add(h);
    }

    private void verificarAcesso(Solicitacao sol, UsuarioPrincipal principal) {
        if (principal.admin()) return;
        boolean ehDono = sol.getAlunoId() != null && sol.getAlunoId().equals(principal.id());
        boolean podeVer = List.of("ORIENTADOR","COMISSAO","SECRETARIA","COORDENADORIA",
                "COORDENACAO","ADMIN").contains(principal.role());
        if (!ehDono && !podeVer)
    ...
    private void verificarDono(Solicitacao sol, UsuarioPrincipal principal) {
        if (principal.admin()) return;
        if (sol.getAlunoId() == null || !sol.getAlunoId().equals(principal.id()))
    ...
    private void atualizarCreditosAluno(String alunoId) {
        if (alunoId == null) return;
        usuarioV2Repo.findById(alunoId).ifPresent(u -> {
            u.setCreditosAprovados(calcularCreditosAprovados(alunoId));
    ...
    public SolicitacaoResponse toResponse(Solicitacao s) {
        ...
        r.setId(s.getId() != null ? Math.abs(s.getId().hashCode()) : 0);
        r.setIdStr(s.getId());
        ...
        r.setNomeAluno(s.getNomeAluno());
        r.setAlunoId(s.getAlunoId() != null ? Math.abs(s.getAlunoId().hashCode()) : null);
