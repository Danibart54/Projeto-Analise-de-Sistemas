...
public class SolicitacaoController {
    ...
    @GetMapping
    @PreAuthorize("hasAnyRole('ORIENTADOR','COMISSAO','SECRETARIA','COORDENADORIA','ADMIN')")
    ...
    @PreAuthorize("hasAnyRole('ORIENTADOR','COMISSAO','SECRETARIA','ADMIN')")
    ...
    @PreAuthorize("hasAnyRole('COMISSAO','COORDENADORIA','ADMIN')")
     ...
     * Só é possível quando a solicitação está em APROVADA_COORDENADORES.
     * Body: { "aprovado": true|false, "justificativa": "..." }
     */
    @PutMapping("/{id}/aval-final")
    @PreAuthorize("hasAnyRole('COMISSAO','ADMIN')")
    public ResponseEntity<SolicitacaoResponse> avalFinal(
            @PathVariable Long id,
            @Valid @RequestBody AvalFinalRequest req,
            @AuthenticationPrincipal UsuarioPrincipal principal) {
        return ResponseEntity.ok(service.darAvalFinal(id, req.isAprovado(), req.getJustificativa(), principal));
    }

    /** DTO inline para o aval final. */
    public static class AvalFinalRequest {
