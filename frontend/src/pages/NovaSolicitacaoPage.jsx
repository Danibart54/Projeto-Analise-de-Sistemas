...
export default function NovaSolicitacaoPage() {
  ...
  // Controle de envio único — previne duplo clique / dupla submissão
  ...
  async function handleSubmit(e) {
    ...
    try {
      /**
       * Correção de cache:
       * 1. Aguardamos o retorno do backend (dados reais salvos no banco).
       * 2. Navegamos para a página do objeto criado usando o ID real retornado.
       * 3. A página de detalhe fará uma nova requisição GET com cache busting,
       *    garantindo que os dados exibidos sejam sempre os mais recentes.
       */
      const solicitacaoCriada = await criarSolicitacao({
      ...
      exibirToast('sucesso', '✅ Solicitação criada com sucesso!')

      // Aguarda um momento para o toast ser visto, depois navega
      setTimeout(() => navigate(`/solicitacoes/${solicitacaoCriada.id}`), 1200)
    } catch (err) {
      exibirToast('erro', err.message || 'Erro ao criar solicitação. Tente novamente.')
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto' }}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        {erros.geral && (
          <div style={s.erroGeral} role="alert">{erros.geral}</div>
        )}

        <div style={s.actions}>
          <button type="button" onClick={() => navigate('/solicitacoes')}
            style={s.cancelBtn} disabled={loading}>
            Cancelar
          </button>
